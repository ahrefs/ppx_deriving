...
For example, if you are using ocamlbuild, add the following to `_tags` to use the default _deriving_ plugins:
    ...
    <src/*>: package(ppx_deriving_melange.std)

With Dune, you should add a `preprocess` directive to your target:

    (executable
      (libraries whatever)
      (preprocess (pps ppx_deriving_melange.show ppx_deriving_melange.ord))
      (name blah))
...
If you are using another buildsystem, just make sure it passes `-package ppx_deriving_melange.whatever` to ocamlfind.
...
$ ocamlfind opt -predicates ppx_driver -package ppx_deriving_foo -package ppx_deriving_bar \
                -package ppx_deriving_melange.main -linkpkg -linkall -o ppx_driver
```
...
requires(ppx_driver) = "ppx_deriving_melange.api"
...
let () = dispatch (
  ...
  | After_rules ->
    (* Assuming files tagged with deriving_foo are already tagged with
       package(ppx_deriving) or anything that uses it, e.g. package(ppx_deriving_melange.std). *)
